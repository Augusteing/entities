...
def scan_file(path: Path) -> Tuple[Tuple[int,int,int], Tuple[int,int,int]]:
    ...
    兼容两种结构:
    1) {"entities": [...], "relations": [...]}  (单对象)
    2) [ {"entities": [...], "relations": [...]}, {...}, ... ] (列表, 多段结果)
    其他结构则忽略。
    """
    try:
...
def main():
    ...
    # 期望结构: <workspace>/抽取/评估/code/本脚本
    # script_dir = .../抽取/评估/code
    # script_dir.parent = .../抽取/评估
    # script_dir.parent.parent = .../抽取  (我们需要的根)
    # 之前错误使用 parents[2] 指到了工作区根, 导致默认 JSON 路径缺少 '抽取' 层级。
    if script_dir.parent.name == '评估' and script_dir.parent.parent.name == '抽取':
        root_dir = script_dir.parent.parent  # 指向 抽取
    else:
        # 回退: 尝试在上级层里查找 '抽取' 目录
        candidates = [p for p in script_dir.parents if (p / '评估').exists() and (p / '数据结果').exists()]
        # 如果没有匹配, 就用 script_dir.parents[1] 保守退回
        root_dir = script_dir.parents[1]
    print(f"[INFO] 解析 root_dir = {root_dir}")
    ...
    if args.out_dir:
        ...
        # 适配实际目录结构：优先使用 顶层 <抽取>/实验过程图/正确率
        top_level_plot_dir = root_dir / '实验过程图'
        if top_level_plot_dir.exists():
            out_dir = top_level_plot_dir / '正确率'
            chosen_mode = '检测到顶层实验过程图目录'
        else:
            # 回退到旧结构 <抽取>/评估/实验过程图/正确率
            out_dir = root_dir / '评估' / '实验过程图' / '正确率'
            chosen_mode = '回退到评估子目录结构'
    out_dir.mkdir(parents=True, exist_ok=True)
    print(f"[INFO] 输出目录: {out_dir} (模式: {chosen_mode})")
