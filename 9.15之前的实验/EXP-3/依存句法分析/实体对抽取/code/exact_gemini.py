...
PAPERS_DIR = os.path.join(BASE_DIR, "依存句法结果")
...
papers = sorted(
    ...
    if f.endswith("_dependency.json")
)
print(f"找到 {len(papers)} 个依存句法结果文件：{papers}")
...
for paper_file in papers:
    # 输出文件路径（支持断点续跑）：去除 _dependency.json 后缀
    if paper_file.endswith("_dependency.json"):
        output_name = paper_file[: -len("_dependency.json")] + ".json"
    else:
        output_name = os.path.splitext(paper_file)[0] + ".json"
    output_file = os.path.join(EXTRACT_RESULT_DIR, output_name)
    ...
    # 读取依存句法 JSON，并拼接为“标题+摘要+正文句子”的文本
    with open(paper_path, "r", encoding="utf-8") as f:
        try:
            obj = json.load(f)
        except Exception as je:
            raise RuntimeError(f"无法解析 JSON：{paper_path} -> {je}")

    title = (obj.get("title") or os.path.splitext(paper_file)[0]).strip()
    summary = (obj.get("summary") or "").strip()
    sentences = []
    for s in obj.get("analyzed_sentences", []) or []:
    ...
    # 填充 prompt（占位符替换；实验一无 schema，占位符留空以兼容旧模板）
    prompt_filled = (
        prompt_template
    ...
    print(f"提交依存句法结果：{paper_file} -> 生成输入文本（长度 {len(paper_text)}）...")
    start_ts = time.time()
    attempts = 0
    ...
    for attempt in range(max_retries):
        attempts += 1
        try:
            # 默认尝试使用 response_format 强制 JSON 数组；若服务端不支持将捕获后降级
            try:
                ...
                # 兼容部分网关不支持 json_schema/response_format 的情况
                if "response_format" in msg_first.lower() or "unsupported" in msg_first.lower() or "json_schema" in msg_first.lower():
                    resp = client.chat.completions.create(
                        model=MODEL_NAME,
                        messages=[
                            {"role": "system", "content": "你是信息抽取助手，只输出严格的 JSON，不要添加多余文本。"},
