...
EXTRACT_RESULT_DIR = os.path.join(DATA_RESULT_DIR, "提取结果_by_kimi")  # JSON结果存放目录（根级）
LOG_DIR = os.path.join(DATA_RESULT_DIR, "log_by_kimi")              # 日志文件存放目录
...
if not os.path.exists(PROMPT_FILE):
    raise FileNotFoundError(f"未找到提示词模板: {PROMPT_FILE} (当前 ROOT={ROOT_DIR})")
...
for paper_file in papers:
    ...
    with open(paper_path, "r", encoding="utf-8") as f:
        ...
        # 加载 schema 内容（修正路径，指向 ROOT/schema文件/schema.txt）
