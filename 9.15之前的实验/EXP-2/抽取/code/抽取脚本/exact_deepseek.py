...
# ------------------------------
# 路径配置（修正为 抽取 根目录；支持 EXTRACT_ROOT 环境变量覆盖）
# ------------------------------
CODE_DIR = os.path.dirname(os.path.abspath(__file__))
CODE_PARENT = os.path.dirname(CODE_DIR)     # 抽取/code
ROOT_DIR = os.path.dirname(CODE_PARENT)     # 抽取
_env_root = os.getenv("EXTRACT_ROOT")
if _env_root and os.path.isdir(_env_root):
    ROOT_DIR = os.path.abspath(_env_root)

PROMPT_FILE = os.path.join(ROOT_DIR, "prompt", "prompt.txt")
PAPERS_DIR = os.path.join(ROOT_DIR, "论文文献")
SCHEMA_DIR = os.path.join(ROOT_DIR, "schema文件")
DATA_RESULT_DIR = os.path.join(ROOT_DIR, "数据结果")
EXTRACT_RESULT_DIR = os.path.join(DATA_RESULT_DIR, "提取结果_by_deepseek")
LOG_DIR = os.path.join(DATA_RESULT_DIR, "log_by_deepseek")
MAIN_LOG_FILE = os.path.join(LOG_DIR, "extraction_log.ndjson")
TIMING_FILE = os.path.join(LOG_DIR, "timings.ndjson")
...
if not os.path.exists(PROMPT_FILE):
    raise FileNotFoundError(f"未找到提示词模板: {PROMPT_FILE} (ROOT={ROOT_DIR})")
...
if not os.path.isdir(PAPERS_DIR):
    raise FileNotFoundError(f"论文目录不存在: {PAPERS_DIR}，请确认 ROOT 是否正确。")
papers = sorted(f for f in os.listdir(PAPERS_DIR) if f.endswith(".md"))
print(f"[路径确认] ROOT={ROOT_DIR}\n  PROMPT_FILE={PROMPT_FILE}\n  PAPERS_DIR={PAPERS_DIR}\n  EXTRACT_RESULT_DIR={EXTRACT_RESULT_DIR}")
...
for paper_file in papers:
    ...
    # 加载 schema 内容（修正为 ROOT/schema文件/schema.txt）
    schema_file = os.path.join(SCHEMA_DIR, "schema.txt")
