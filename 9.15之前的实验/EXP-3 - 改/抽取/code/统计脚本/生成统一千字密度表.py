...
def build_paths(root: Path):
    ...
    # 使用完整的提取结果目录（不依赖 in_scope 子目录），具体统计范围由 papers_dir 的 50 篇 stem 决定
    model_dirs = {
        'deepseek': root / '数据结果' / '提取结果_by_deepseek',
        'gemini':   root / '数据结果' / '提取结果_by_gemini',
        'kimi':     root / '数据结果' / '提取结果_by_kimi',
    }
    out_csv = root / '数据结果' / '按论文模型_实体关系千字密度_统一口径.csv'
    # 额外导出：按模型汇总的千字密度均值
    out_models_csv = root / '数据结果' / '模型千字密度均值.csv'
    return papers_dir, model_dirs, out_csv, out_models_csv
...
def main():
    args = parse_args()
    papers_dir, model_dirs, out_csv, out_models_csv = build_paths(args.root)
    ...
    per_model_stats = {
        m: {
            ...
            'total_relations': 0,
            'total_clean_len': 0,
            'sum_ent_density': 0.0,  # 简单均值：逐论文密度求和
            'sum_rel_density': 0.0,
    ...
    for stem in stems:
        ...
        for model, mdir in model_dirs.items():
            ...
            # 按模型聚合
            s = per_model_stats[model]
            s['paper_count'] += 1
            s['total_entities'] += ent_cnt
            s['total_relations'] += rel_cnt
            s['total_clean_len'] += cleaned_len
            s['sum_ent_density'] += ent_density
            s['sum_rel_density'] += rel_density

    out_csv.parent.mkdir(parents=True, exist_ok=True)
    # 安全写入：若目标被占用(如 Excel 打开)则退避到带时间戳的新文件
    out_csv_real = out_csv
    try:
        ...
        out_csv_real = out_csv.with_name(out_csv.stem + f'_{ts}' + out_csv.suffix)
        with out_csv_real.open('w', newline='', encoding='utf-8-sig') as f:
            w = csv.writer(f)
            w.writerow([
                '论文stem','模型','原始字符数','去空白字符数','实体数量','关系数量',
                '实体千字密度(按去空白)','关系千字密度(按去空白)','关系/实体比'
    ...
    # 输出按模型的千字密度均值（加权与简单均值）
    out_models_csv.parent.mkdir(parents=True, exist_ok=True)
    out_models_csv_real = out_models_csv
    try:
        with out_models_csv.open('w', newline='', encoding='utf-8-sig') as f:
            w = csv.writer(f)
            w.writerow([
                '模型','论文数','总实体','总关系','总去空白字符数',
                '实体千字密度(加权)','关系千字密度(加权)',
                '实体千字密度(简单均值)','关系千字密度(简单均值)',
                '关系/实体比(加权)'
            ])
            for model, s in per_model_stats.items():
                pl = max(s['total_clean_len'], 1)
                ent_density_weighted = s['total_entities'] * 1000 / pl
                rel_density_weighted = s['total_relations'] * 1000 / pl
                pc = max(s['paper_count'], 1)
                ent_density_simple = s['sum_ent_density'] / pc
                rel_density_simple = s['sum_rel_density'] / pc
                ratio_weighted = (s['total_relations'] / s['total_entities']) if s['total_entities'] else 0
                w.writerow([
                    model, s['paper_count'], s['total_entities'], s['total_relations'], s['total_clean_len'],
                    f"{ent_density_weighted:.4f}", f"{rel_density_weighted:.4f}",
                    f"{ent_density_simple:.4f}", f"{rel_density_simple:.4f}",
                    f"{ratio_weighted:.4f}"
                ])
    except PermissionError:
        ts = datetime.now().strftime('%Y%m%d_%H%M%S')
        out_models_csv_real = out_models_csv.with_name(out_models_csv.stem + f'_{ts}' + out_models_csv.suffix)
        with out_models_csv_real.open('w', newline='', encoding='utf-8-sig') as f:
            w = csv.writer(f)
            w.writerow([
                '模型','论文数','总实体','总关系','总去空白字符数',
                '实体千字密度(加权)','关系千字密度(加权)',
    ...
    print('已生成统一口径 CSV ->', out_csv_real)
    print('已生成模型千字密度均值 CSV ->', out_models_csv_real)
    ...
    for model, s in per_model_stats.items():
        ...
        ent_density_simple = s['sum_ent_density'] / pc
        rel_density_simple = s['sum_rel_density'] / pc
        print(f"  {model}: 加权(实体/关系)={ent_density_weighted:.4f}/{rel_density_weighted:.4f} | 简单(实体/关系)={ent_density_simple:.4f}/{rel_density_simple:.4f} | 论文数={s['paper_count']}")
    print('\n参数确认:')
